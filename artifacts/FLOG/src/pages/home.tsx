    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <div className="w-full relative shrink-0" style={{ paddingBottom: "66.67%" }}>
            {item.imageUrl ? (
              <img
                src={item.imageUrl}
                alt={item.title}
...
export default function Home() {
  ...
  const goToInterviewPage = (p: number) => {
    setInterviewPage(p);
    setTimeout(() => {
  ...
  return (
    <>
      ...
      <section className="bg-white py-12 md:py-16 px-5 md:px-12">
        ...
        <div id="interviews-section" ref={interviewSectionRef}>
          <div className="mb-6 md:mb-8">
            <h2 className="font-serif text-[28px] md:text-[42px] font-bold tracking-tight text-foreground leading-none mb-3 md:mb-4">
              INTERVIEWS
            </h2>
          ...
          {/* Pagination — visible when 2+ pages */}
          {!loading && totalInterviewPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-12 md:mt-16">
              {Array.from({ length: totalInterviewPages }).map((_, p) => (
                <button
                  key={p}
                  onClick={() => goToInterviewPage(p)}
                  className={`w-9 h-9 font-sans text-[12px] tracking-wide transition-colors ${
                    p === interviewPage
                      ? "bg-foreground text-background"
                      : "text-foreground/40 hover:text-foreground border border-foreground/20 hover:border-foreground/50"
                  }`}
                >
                  {p + 1}
                </button>
